...
# Liste des termes impératifs
termes_imperatifs = [
    "doit", "est tenu de", "obligatoire", "conformément à", "interdit",
    "non conforme", "il est impératif de", "est requis", "obligation de",
    "imposé par", "conforme à la norme", "sanctionné par", "en infraction avec",
    "circulaire", "exigé par la commission bancaire", "exigé par le superviseur",
    "exigé par le régulateur", "se conformer à", "respecter", "instruction",
    "ne doit pas", "doit être"
]

mots_abstraits = [
    "gérer", "organiser", "coordonner", "définir", "établir", "optimiser", "concevoir", "planifier",
    "analyser", "mettre en œuvre", "implémenter", "structurer", "configurer", "assurer", "piloter",
    "conduire", "déployer", "améliorer", "évaluer", "suivre"
]
verbes_techniques = [
    "compiler", "scripter", "virtualiser", "configurer", "déboguer", "crypter", "cloner", "monitorer",
    "tokeniser", "parser", "indexer", "authentifier"
]
# Liste de verbes abstraits (à enrichir)
verbes_abstraits = set([
    "gérer", "assurer", "optimiser", "superviser", "soutenir",
    "améliorer", "maintenir", "contribuer", "favoriser"
])

# Liste de mots flous (à enrichir aussi)
mots_flous = set([
    "correctement", "efficacement", "de manière appropriée",
    "optimal", "pertinent", "approprié", "bon fonctionnement"
])

verbes_facilitants = [
    "mettre en place", "vérifier", "sécuriser", "formaliser", "documenter", "renforcer", "centraliser",
    "supprimer", "auditer", "remédier", "corriger", "justifier", "s'assurer", "établir", "limiter",
    "instaurer", "réaliser", "clarifier", "régulariser", "identifier", "rédiger", "modifier",
    "procéder à", "activer", "concevoir", "diffuser", "revoir", "déployer", "mettre à jour",
    "implémenter", "élaborer", "contrôler", "étayer"
]

ressources_keywords = [
    "ordinateur", "logiciel", "application", "formulaire", "fichier", "document", "base de données",
    "matériel", "réseau", "support", "connexion", "email", "identifiant", "dossier", "clé USB", "serveur"
]


subordonnants = [
    "si", "lorsque", "quand", "afin que", "bien que", "parce que", "puisque", "avant que", "après que",
    "pendant que", "comme", "alors que", "tandis que"
]
...
def score_comprehension(texte: str) -> dict:
    ...
    score_longueur = normaliser(longueur_moyenne, 30)           # seuil max : 30 mots/phrase
    score_complexite = normaliser(complexite, 10)               # seuil max : 10 sub/passives
    score_vocabulaire = normaliser(mots_difficiles, 15)         # seuil max : 15 mots difficiles

    # Nombre de fautes
    erreurs = outil_langue.check(texte)
    nb_fautes = len(erreurs)
    score_fautes = normaliser(nb_fautes, 10) 
    # Moyenne pondérée
      score_global = round((
        0.15 * score_longueur +
        0.30 * score_complexite +
        0.25 * score_vocabulaire +
        0.20 * score_lisibilite +
        0.10 * score_fautes
    ), 2)
...
def evaluer_complexite(tache):
    ...
    nb_subordonnees = sum(1 for token in doc if token.dep_ in ["mark", "advcl", "relcl"])
    ...
    score = (
        ...
        0.2 * (nb_verbes / 3) +
        0.2 * (nb_subordonnees / 2) +
        0.2 * (nb_abstraits / 2) +
        0.2 * (1 - lisibilite_score)      # plus difficile à lire = + complexe
    )
    return score
...
def AnalyseTache(taches):
    for tache in taches:
        ...
        # Acteurs probables (par heuristique : contient "Manager", "Direction", etc.)
          if any(mot in texte.lower() for mot in ["manager", "direction", "responsable", "service"]):
            acteurs.add(texte)
        # Ressources probables
          elif any(mot in texte.lower() for mot in ["contrôle", "état", "transmission", "provision", "charge", "perte"]):
            ressources.add(texte)

        for token in doc:
          if token.dep_ in ("nsubj", "nmod", "pobj", "appos") and token.pos_ in ("PROPN", "NOUN"):
               acteurs.add(token.text)
          if token.pos_ == "VERB":
               actions.add(token.lemma_)  # le verbe à l'infinitif
          if token.dep_ in ("obj", "obl", "nmod") and token.pos_ in ("NOUN", "PROPN"):
               ressources.add(token.text)

        print("🔸 Phrase analysée :", tache)
        print("👤 Personnes concernées :", ", ".join(acteurs) or "Aucune trouvée")
        print("✅ Verbes d’action :", ", ".join(actions) or "Aucun trouvé")
        print("📦 Ressources associées :", ", ".join(ressources) or "Aucune trouvée")
...
def extraire_taches(texte: str) -> list[str]:
    ...
    for phrase in doc.sents:
        ...
        for token in phrase_doc:
            if token.pos_ == "VERB" and token.morph.get("VerbForm") in [["Inf"], ["Imp"]]:
...
def evaluer_difficulte_tache(tache: str) -> dict:
    ...
    for verbe in verbes_facilitants:
        if verbe in texte_min:
            score -= 10
            # 2. Verbes facilitants = difficulté ↓
    for impe in termes_imperatifs:
        if impe in texte_min:
            score -= 10
