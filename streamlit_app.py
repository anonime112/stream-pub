...
colonne=['criticité reco','statut de mise en oeuvre', 'écheance initiale', 'écheance revisée', 'priorité pa','% avancement']
...
def comparer_a_aujourdhui(date_str, format_date=None):
    try:
        ...
        if format_date:
            date_donnee = pd.to_datetime(date_str, format=format_date, errors='coerce')
        else:
...
def remplacer_variantes(df, colonne, enumeration):
    ...
    def nettoyer_valeur(val):
        ...
        return mapping.get(val_norm, val)  # Remplace si correspondance, sinon garde la valeur d'origine
...
def verifier_valeur(val, dtype_attendu):
    ...
    if pd.api.types.is_string_dtype(dtype_attendu):
        ...
        return isinstance(val, int) and not isinstance(val, bool)  # bool est sous-classe de int
...
def afficher_types_colonnes_strict(df: pd.DataFrame):
    ...
    styled_df = df_types.style.apply(
        lambda col: [colorier_cellule(v, col.name) for v in col],
        axis=0
    )
...
def afficher_types_colonnes(df: pd.DataFrame):
    for col in df.columns:
        ...
        for i, val in enumerate(df[col]):
            ...
            st.write(f"Ligne {i} - Valeur : `{val}` (type réel : {val_type}) ➤ Correspondance : {match}")
...
def formulaire_par_colonne(df: pd.DataFrame, ligne_index: int = 0):
    ...
    for col in df.columns:
        ...
        user_input = st.text_input(f"Entrer une valeur pour '{col}'", value=str(val), key=col)
        st.write("---")          
...
def ajouter_colonne_calcul(df: pd.DataFrame, nom_colonne="Score Total"):
 ...
 # Assumons que les colonnes à additionner sont les 4e et 5e (index 3 et 4 en Python)
    ...
    df['Somme'] = df[colonne_4] + df[colonne_5]
    ...
    styled_df = df.style.applymap(colorier, subset=['Somme'])

    st.success(f"Colonne '{nom_colonne}' ajoutée avec la somme de {colonnes_numeriques[:2]}")
    st.dataframe(styled_df)
    edited_df = st.data_editor(styled_df, num_rows="dynamic", use_container_width=True)
    
    st.write("Cliquez sur une ligne :")
    for i in styled_df.index:
        if st.button(f"Sélectionner la ligne {i}"):
            st.write(df.loc[i])
...
def verifie_statut_avancement():
    
    print('satut')


def verifie_criticiter():
    
    print('satut')

def verifie_echeance():
...
if uploaded_files is not None:
    ...
    if colonnes_absentes:
        st.error(f"Les colonnes suivantes sont manquantes : {', '.join(colonnes_absentes)}")
    else:
    # ✅ Toutes les colonnes sont présentes, on peut exécuter du code
        ...
        df5=modifier_colonne_si_enum(df4,"statut de mise en oeuvre","% avancement",stat_contro2,"100%" )
        df5=modifier_colonne_si_enum(df5,"statut de mise en oeuvre","% avancement",stat_control,"0%" )
